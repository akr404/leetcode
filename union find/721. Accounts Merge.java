...
class Solution {
  public List<List<String>> accountsMerge(List<List<String>> accounts) {
    ...
    //if it is different, it means that it has some other parent.  Basically we are mapping each account to its index. 
    //And at the start, every account is its own parent.
    int[] parents = new int[accounts.size()];
    for (int i = 0; i < accounts.size(); i++) {
      parents[i] = i;  
    }
    // This hashmap contains the email to index mapping. 
    // We will use this map to find the particular account when an email has already been found.
    ...
    for (int i = 0; i < accounts.size(); i++) {
      ...
      for (int j = 1; j < accounts.get(i).size(); j++) {
        ...
        if (owners.containsKey(email)) {
          ...
          // If they weren't already connected, then connect them. 
          // For connecting them, we would reassign the index of the account of where we found our email to the current account we are processing.
          if (p1 != p2) {
            parents[p2] = p1;  
          }
        } else {
